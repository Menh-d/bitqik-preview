	...
	Codevz_Plus.grid = function( wpb ) {
		...
		$( '.cz_grid' ).not( '.slick, .slick .cz_grid' ).codevzPlus( 'grid', function( x ) {
			...
			if ( x.hasClass( 'cz_justified' ) ) {
				...
				// 1. The Core Layout Logic
				window.czJustifyLayout = function() {

					gridAnimation();
				    x.addClass('cz_grid_no_loading');

					const justifyGrids = document.querySelectorAll('.cz_justified');

				    justifyGrids.forEach(grid => {
				        const items = Array.from(grid.querySelectorAll('.cz_grid_item:not(.cz_hidden_item)'));
				        ...
				        const targetHeight = parseInt(grid.getAttribute('data-height')) || 300;
				        
				        const firstItemStyle = window.getComputedStyle(items[0]);
				        const gap = parseInt(firstItemStyle.getPropertyValue('--codevz-justified-gap')) || 0;
				        ...
				        items.forEach((item, index) => {
				            ...
				            } else if (img && img.getAttribute('width') && img.getAttribute('height')) {
				                ratio = parseInt(img.getAttribute('width')) / parseInt(img.getAttribute('height'));
				            }
				            
				            const itemWidthAtTarget = targetHeight * ratio;
				            rowItems.push({ el: item, ratio: ratio, width: itemWidthAtTarget });
				            currentRowWidth += itemWidthAtTarget + gap;

				            const isLastOverall = index === items.length - 1;
				            const threshold = isMobile ? containerWidth : containerWidth * 1.15;

				            if (currentRowWidth - gap > threshold || isLastOverall) {
				                let shouldJustify = true;
				                if (!isMobile && isLastOverall) {
				                    const naturalRowWidth = rowItems.reduce((a, b) => a + b.width, 0) + ((rowItems.length - 1) * gap);
				                    if (naturalRowWidth < containerWidth * 0.7) {
				                        shouldJustify = false;
				                    }
				                }

				                let rowHeight = targetHeight;
				                const totalRatio = rowItems.reduce((sum, ri) => sum + ri.ratio, 0);
				                const availableWidthForItems = containerWidth - ((rowItems.length - 1) * gap);

				                if (shouldJustify) {
				                    rowHeight = availableWidthForItems / totalRatio;
				                }
				                ...
				                rowItems.forEach((rowItem, i) => {
				                    ...
				                    if (!shouldJustify) {
				                        w = Math.floor(rowItem.ratio * targetHeight);
				                        h = targetHeight;
				                    } else {
				                        if (isLastInRow) {
				                            w = containerWidth - usedWidth - ((rowItems.length - 1) * gap);
				                        } else {
				                            w = Math.floor(rowItem.ratio * rowHeight);
				                            usedWidth += w;
				                        }
				                        h = Math.floor(rowHeight);
				                    }

				                    rowItem.el.style.width = w + 'px';
				                    rowItem.el.style.height = h + 'px';
				                    rowItem.el.style.marginBottom = gap + 'px';
				                    if ( document.body.classList.contains( 'rtl' ) ) {
				                    	rowItem.el.style.marginLeft = isLastInRow ? '0px' : gap + 'px';
				                    } else {
				                    	rowItem.el.style.marginRight = isLastInRow ? '0px' : gap + 'px';
				                    }
				                });
				...
				const init = () => {
				    const grids = document.querySelectorAll('.cz_justified');
				    grids.forEach(grid => {
				        if (typeof imagesLoaded !== 'undefined') {
				            const imgLoad = imagesLoaded(grid);
				            imgLoad.on('progress', debounce(window.czJustifyLayout, 50));
				            imgLoad.on('always', window.czJustifyLayout);
				        } else {
				            window.czJustifyLayout();
				        }
				    });

				    setTimeout(window.czJustifyLayout, 500);
				    setTimeout(window.czJustifyLayout, 1500);
				};
				...
				window.addEventListener('resize', debounce(window.czJustifyLayout, 150));

				// 2. Your Filter Implementation
				document.querySelectorAll('.cz_justified').forEach(grid => {
				  const filterList = grid.previousElementSibling;
				  if (!filterList || filterList.tagName !== 'UL') return;

				  filterList.querySelectorAll('li').forEach(li => {
				    li.onclick = () => {
				      li.classList.add('cz_active_filter');
				      [...li.parentElement.children].forEach(sib => sib !== li && sib.classList.remove('cz_active_filter'));

				      const filter = li.getAttribute('data-filter');
				      const items = grid.querySelectorAll('.cz_grid_item');

				      items.forEach(item => {
				        const isMatch = !filter || filter === '*' || item.matches(filter);
				        
				        item.classList.toggle('cz_hidden_item', !isMatch);
				        item.style.display = isMatch ? '' : 'none';
				        item.style.opacity = isMatch ? '1' : '0.8';

				        if (isMatch) {
				          item.animate([{ opacity: 0.8 }, { opacity: 1 }], { duration: 250, fill: 'forwards' });
				        }
				      });
				...
				function throttleIsotopeLayout(isotopeInstance, container) {
					...
					isotopeTimeout = setTimeout(function runIsotopeLayout() {
						...
						setTimeout( function() {
							isotopeInstance.isotope('layout');
							$( window ).trigger( 'resize' ).trigger( 'scroll.grid_animation' );
						}, 250 );
					...
					$.ajax({
						...
						success: function( data ) {
							...
							if ( x.hasClass( 'cz_justified' ) ) {
								...
								x.append( data );
								imagesLoaded( '.cz_justified' ).on( 'progress', function() {
									window.czJustifyLayout();
								});
